...
<template>
    <div class="overflow-x-auto">
        <h4 class="text-lg font-semibold mb-4 text-theme-text-primary">User Like Statistics</h4>
        ...
        <table class="min-w-full divide-y divide-theme-border border border-theme-border">
            <thead class="bg-theme-elevated/50">
                <tr>
                    <th class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-theme-text-primary sm:pl-6 sticky left-0 bg-theme-elevated/50 z-10">Liker</th>
                    <th v-for="user in likeStats.users" :key="user.id" class="px-3 py-3.5 text-center text-sm font-semibold text-theme-text-primary whitespace-nowrap">
                        {{ user.name }}
                    </th>
                </tr>
            </thead>
            <tbody class="divide-y divide-theme-border">
                <tr v-for="row in likeStats.stats" :key="row.likerId">
                    <td class="whitespace-nowrap py-4 pl-4 pr-3 text-sm font-medium text-theme-text-primary sm:pl-6 sticky left-0 bg-theme-card z-10">{{ row.likerName }}</td>
                    <td v-for="user in likeStats.users" :key="user.id" class="whitespace-nowrap px-3 py-4 text-sm text-center text-theme-text-primary" :class="{'!text-theme-text-muted': row.likes[user.id] === 0, 'font-bold': row.likes[user.id] > 0}">
                        {{ row.likes[user.id] }}
                    </td>
                </tr>
            </tbody>

        ...
        <div class="bg-theme-page/50 p-4 rounded-lg mb-6">
            <form @submit.prevent="submitMeetup">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <input v-model="form.title" placeholder="Meetup Title" class="w-full bg-theme-card border-theme-border rounded-md" required />
                        <p v-if="form.errors.title" class="mt-1 text-sm text-red-400">{{ form.errors.title }}</p>
                    </div>
                    <div>
                        <input v-model="form.type" placeholder="Meetup Type (e.g., Run, Gym)" class="w-full bg-theme-card border-theme-border rounded-md" />
                        <p v-if="form.errors.type" class="mt-1 text-sm text-red-400">{{ form.errors.type }}</p>
                    </div>
                    <div class="md:col-span-2">
                        <textarea v-model="form.description" placeholder="Description" class="w-full bg-theme-card border-theme-border rounded-md"></textarea>
                        <p v-if="form.errors.description" class="mt-1 text-sm text-red-400">{{ form.errors.description }}</p>
                    </div>
                    <div class="md:col-span-2">
                        <input v-model="form.location" placeholder="Location (e.g., Central Park)" class="w-full bg-theme-card border-theme-border rounded-md" required />
                        <p v-if="form.errors.location" class="mt-1 text-sm text-red-400">{{ form.errors.location }}</p>
                    </div>
                    <div>
                        <input v-model="form.latitude" type="number" step="any" placeholder="Latitude" class="w-full bg-theme-card border-theme-border rounded-md" />
                        <p v-if="form.errors.latitude" class="mt-1 text-sm text-red-400">{{ form.errors.latitude }}</p>
                    </div>
                    <div>
                        <input v-model="form.longitude" type="number" step="any" placeholder="Longitude" class="w-full bg-theme-card border-theme-border rounded-md" />
                        <p v-if="form.errors.longitude" class="mt-1 text-sm text-red-400">{{ form.errors.longitude }}</p>
                    </div>
                    <div class="md:col-span-2">
                        <input v-model="form.scheduled_at" type="datetime-local" class="w-full bg-theme-card border-theme-border rounded-md" required />
                        <p v-if="form.errors.scheduled_at" class="mt-1 text-sm text-red-400">{{ form.errors.scheduled_at }}</p>
                    </div>
                </div>
                <div class="mt-4 flex items-center space-x-2">
                    <button type="submit" :disabled="form.processing" class="px-3 py-1.5 text-sm bg-theme-accent hover:bg-theme-accent-hover rounded-md disabled:opacity-50">{{ editingMeetup ? 'Update Meetup' : 'Create Meetup' }}</button>
                    <button v-if="editingMeetup" @click="cancelEdit" type="button" class="px-3 py-1.5 text-sm bg-theme-elevated hover:bg-theme-elevated rounded-md">Cancel</button>
                </div>
            </form>
        </div>
